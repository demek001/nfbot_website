...
//                                    (sem texto: o código nunca vai na URL)
...
Deno.serve(async (req) => {
  ...
  try {
    if (modo === "wa") {
      // clique: incrementa e redireciona. O código de ativação NÃO entra na URL —
      // o e-mail de cortesia já traz "ATIVAR {codigo}" num bloco copiável.
      const r = await fetch(
        `${SUPABASE_URL}/rest/v1/emails_log?id=eq.${id}&select=cliques,clicado_em,cliente_id`,
        { headers: sb() },
      );
      ...
      return Response.redirect(`https://wa.me/${NUMERO_WA}`, 302);
    }

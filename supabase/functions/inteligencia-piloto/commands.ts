...
export async function comandoPiloto(cliente:any,texto:string,rpc:Rpc,send:(s:string)=>Promise<any>):Promise<boolean> {
  const t=texto.normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim().toLowerCase();
  if(!/^(inteligencia|assinaturas|recorrencias|meus lembretes|pausar alertas|posso gastar(?:\s.*)?)\??$/.test(t)) return false;
  ...
  if(t.replace(/\?$/,'')==='inteligencia') {
    ...
    await send(Array.isArray(r)?(r.length?r.map((i:any)=>`${i.descricao} — ${new Date(i.devido_em).toLocaleDateString('pt-BR',{timeZone:'America/Sao_Paulo'})}`).join('\n'):'Você ainda não tem lembretes cadastrados.'):'Não consegui consultar seus lembretes.');
  } else {

...
function labelMes(anoMesStr: string): string { return anoMes(`${anoMesStr}-01`).amigavel; }
async function iniciarPlanilhaMes(cliente: any, phoneNumberId: string, from: string) {
  const notas = await sbSelect(`notas_fiscais?cliente_id=eq.${cliente.id}&data_compra=not.is.null&select=data_compra`);
  const cont = new Map<string, number>();
  (Array.isArray(notas) ? notas : []).forEach((n: any) => { const k = anoMesKey(n.data_compra); cont.set(k, (cont.get(k) ?? 0) + 1); });
  const meses = [...cont.keys()].sort().reverse().slice(0, 12);
  if (meses.length === 0) { await enviarWhats(phoneNumberId, from, "📊 *Planilha do mês*\n\nVocê ainda não tem notas registradas. Manda a *foto* ou *PDF* da sua primeira nota que eu crio sua planilha. 🙂"); return; }
  const linhas = meses.map((m, i) => `${i + 1}) ${labelMes(m)}`).join("\n");
  const ctx = { tipo: "planilha_lista", lista: meses };
  await sbPatch(`clientes?id=eq.${cliente.id}`, { menu_ctx: ctx });
  cliente.menu_ctx = ctx;
  await enviarWhats(phoneNumberId, from, `📊 *Planilha do mês*\nEscolha o mês (responde *só o número*):\n\n${linhas}`);
}
async function enviarPlanilhaMes(cliente: any, phoneNumberId: string, from: string, anoMesStr: string) {
  const { ini, prox } = limitesMes(anoMesStr);
  const notas = await sbSelect(`notas_fiscais?cliente_id=eq.${cliente.id}&data_compra=gte.${ini}&data_compra=lt.${prox}&sheet_id=not.is.null&select=sheet_id`);
  const cont = new Map<string, number>();
  (Array.isArray(notas) ? notas : []).forEach((n: any) => { if (n.sheet_id) cont.set(n.sheet_id, (cont.get(n.sheet_id) ?? 0) + 1); });
  let melhor: string | null = null, max = -1;
  for (const [sid, q] of cont) { if (q > max) { max = q; melhor = sid; } }
  if (!melhor) { await enviarWhats(phoneNumberId, from, `📊 A planilha de ${labelMes(anoMesStr)} ainda não está disponível. Manda uma nota desse mês que eu crio pra você. 🙂`); return; }
...
async function tratarMenuNav(cliente: any, phoneNumberId: string, from: string, texto: string): Promise<boolean> {
  ...
  if (ctx.tipo === "planilha_lista") {
    const lista = Array.isArray(ctx.lista) ? ctx.lista : [];
    if (op < 1 || op > lista.length) {
      const linhas = lista.map((m: string, i: number) => `${i + 1}) ${labelMes(m)}`).join("\n");
      await enviarWhats(phoneNumberId, from, `Esse número não está na lista. Escolha o mês (responde *só o número*):\n\n${linhas}`);
      return true;
    }
    await sbPatch(`clientes?id=eq.${cliente.id}`, { menu_ctx: null });
    cliente.menu_ctx = null;
...
async function tratarMenuPos(cliente: any, phoneNumberId: string, from: string, texto: string): Promise<boolean> {
  ...
  if (op === 2) {
    ...
    else await enviarWhats(phoneNumberId, from, "🏷️ Por favor, responda a foto da nota (use \"Responder\") e escreva a categoria desejada.");
    return true;
  }
  if (op === 3) {
    let nota: any = null;

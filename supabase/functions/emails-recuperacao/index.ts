...
// OAuth do Drive: mesma montagem da função onboarding (state opaco em oauth_states)
...
// O default da tabela é 30 minutos, curto demais para link que viaja por e-mail.
...
// Link de conexão do Drive é POR CLIENTE (o state identifica quem conectou).
// Override manual: config_privada.OAUTH_DRIVE_URL ≠ "PENDENTE" vale pra todos.
// Sem GOOGLE_CLIENT_ID, sem override ou sem conseguir gravar o state, cai na
// página da conta — melhor que mandar um link que o callback vai recusar.
async function linkOauthDrive(clienteId: string, override: string): Promise<string> {
  if (override && override !== "PENDENTE") return override;
  if (!GOOGLE_CLIENT_ID) return CONTA_URL;
  const state = await criarState(clienteId, 24 * 7);  // R3 vai por e-mail
  if (!state) return CONTA_URL;
  return "https://accounts.google.com/o/oauth2/v2/auth?" + new URLSearchParams({
    client_id: GOOGLE_CLIENT_ID, redirect_uri: OAUTH_REDIRECT, response_type: "code",
    scope: OAUTH_SCOPE, access_type: "offline", prompt: "consent", state,
  }).toString();
}
    ...
    for (const c of clientes) {
      ...
      if (!c.ativado) {
        ...
        await enviarRecuperacao(
          ...
          merge(TPL_R3, { primeiro_nome: nome, link_oauth_drive: await linkOauthDrive(c.id, oauthOverride) }), resumo,
        );
      }
    }
